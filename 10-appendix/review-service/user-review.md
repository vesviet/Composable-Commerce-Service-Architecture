...
### 🔴 P0 Issues (Blocking)
1. **[TESTING]** `user/internal/biz` — Unit Test coverage is low (31.9% in `biz/user`, 0% in `biz/events`). Mission-critical admin user management lacks full test coverage, and mocks are manually written instead of using `gomock` as mandated by `testcase.md`.
...
2. **[DATABASE]** `user/internal/data/postgres/user.go` — Uses offset-based pagination (`Offset().Limit()`). For large administrative logs or user tables, this must be refactored to cursor-based (keyset) pagination to prevent DB spikes.
