...
**Created:** 2025-11-19  
**Updated:** 2026-01-18  
**Status:** 🟡 Partially verified vs code (core sellable view implemented; some claims need re-check)  
**Priority:** High
...
4. [Verified vs Planned Snapshot](#verified-vs-planned-snapshot)
5. [Catalog Cache Strategy](#catalog-cache-strategy)
6. [Realtime Analysis](#realtime-analysis)
7. [Search Content Suggestions](#search-content-suggestions)
8. [CMS Page Cache Strategy](#cms-page-cache-strategy)
...
- Gateway service routing and middleware:
  - Search service exists in `gateway/configs/gateway.yaml` and routes include `warehouse_detection`.
- Search service supports per-warehouse sellable view:
  - Reads `X-Warehouse-ID` from headers.
  - Uses `warehouse_stock` field in Elasticsearch docs for filtering/enrichment.
  - Stock/price views update `warehouse_stock` via scripts.

### Planned / needs re-check

- Script sort on `warehouse_stock` is currently marked TODO in code (`search/internal/data/elasticsearch/sort_builder.go`: `TODO: Fix script sort for nested warehouse_stock field`).
- Promotion integration fields (discount/final_price) remain future/planned.
